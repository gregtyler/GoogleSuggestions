...
To answer this question, I gathered names of high profile people with different professions (scientists, tennis players, Hollywood actors), wrote a script to fetch the list of Google suggestions, and compared the proportion of people for whom "husband" and/or "wife" are suggested as a function of their profession; I completed the analysis by also looking for the word "married" (see the [Methods](#Methods) section for more details). 
...
countsuggestions <- function(cat){
	...
	nMwh <- sum(!tab$sex=="F" & tab$wh==1)
	nFm <- sum(tab$sex=="F" & tab$married==1)
	nMm <- sum(!tab$sex=="F" & tab$married==1)
	out <- c(nF=nF, nM=nM, #
...
The difference between male (`r round(xx$nMm[xx$cat=="actors"], 2)`/`r round(xx$nM[xx$cat=="actors"], 2)`) and female (`r round(xx$nFm[xx$cat=="actors"], 2)`/`r round(xx$nF[xx$cat=="actors"], 2)`) actors in terms of the frequency of the "married" suggestion by Google is not significant either. 
```{r}
summary(glm(formula = wh ~ married, data=t.actors, family = binomial))
```

### Tennis players
There is a strong difference between male and female tennis players, but maybe not the one you'd expect: "husband" or "wife" appear in Google's suggestions for `r xx$nFwh[xx$cat=="tennis"]` women out of the `r sum(t.tennis$sex=="F")` in the dataset, while they appear for `r xx$nMwh[xx$cat=="tennis"]` men (out of `r sum(t.tennis$sex=="M")`). One reason may be that, for an equivalent ranking, the popularity of female tennis players is lower than the popularity of male tennis players. 
```{r}
summary(glm(formula = wh ~ sex, data=t.tennis, family = binomial))
```

What about the "married" word in suggestions? It occured for `r round(xx$nMm[xx$cat=="tennis"], 2)` male players and `r round(xx$nFm[xx$cat=="tennis"], 2)` female players; the difference is not significant either.
```{r}
...
```{r}
model.hhmi.s <- glm(formula = wh ~ sex, data=t.science[t.science$HHMI=="yes",], family = binomial)
summary(model.hhmi.s)
```

There is indeed an effect of sex on the proportion of "wife" and "husband" in Google Suggestions for HHMI scientists. But if we look closer, we realize that out of the `r sum(t.science[t.science$sex=="F" & t.science$HHMI=="yes", "wh"])` HHMI female scientists for whom "husband" or "wife" appears in Google's suggestions, `r sum(t.science[t.science$sex=="F" & t.science$HHMI=="yes" & t.science$TED=="yes", "wh"])` have given TED talks, out of the `r length(t.science[t.science$sex=="F" & t.science$HHMI=="yes" & t.science$TED=="yes", "wh"])` female HHMI scientists who have given TED talks. Out of the `r sum(t.science[t.science$sex=="M" & t.science$HHMI=="yes", "wh"])` male HHMI scientists for whom "husband" or "wife" appears in Google's suggestions, `r sum(t.science[t.science$sex=="M" & t.science$HHMI=="yes" & t.science$TED=="yes", "wh"])` have given TED talks, out of the `r length(t.science[t.science$sex=="M" & t.science$HHMI=="yes" & t.science$TED=="yes", "wh"])` male HHMI scientists who have given TED talks.
...
```{r echo=FALSE}
tab <- tapply(t.science[t.science$HHMI=="yes", "wh"], list(sex=t.science[t.science$HHMI=="yes","sex"], ted=t.science[t.science$HHMI=="yes","TED"]), length)
rownames(tab) <- c("M", "W")
colnames(tab) <- c("no", "yes")
tab
```
Now adding participation to a TED talk in the analysis of this HHMI dataset, we obtain:
```{r}
model.hhmi.st <- glm(formula = wh ~ sex + TED, data=t.science[t.science$HHMI=="yes", ], family = binomial)
summary(model.hhmi.st)
```
Testing for an interaction between sex and TED:
```{r}
model.hhmi.i <- glm(formula = wh ~ sex * TED, data=t.science[t.science$HHMI=="yes", ], family = binomial)
summary(model.hhmi.i)
```
and finally TED only:
```{r}
model.hhmi.t <- glm(formula = wh ~ TED, data=t.science[t.science$HHMI=="yes", ], family = binomial)
summary(model.hhmi.t)
```
...
```
```{r}
model.sel(model.hhmi.s, model.hhmi.t, model.hhmi.st, model.hhmi.i)

```
We cannot distinguish between all the models that take TED talks into account, but the model with "sex" only is the least good. 

### AAAS data

```{r}
model.aaas.s <- glm(formula = wh ~ sex, data=t.science[t.science$AAAS=="yes", ], family = binomial)
model.aaas.t <- glm(formula = wh ~ TED, data=t.science[t.science$AAAS=="yes", ], family = binomial)
model.aaas.st <- glm(formula = wh ~ sex + TED, data=t.science[t.science$AAAS=="yes", ], family = binomial)
model.aaas.i <- glm(formula = wh ~ sex * TED, data=t.science[t.science$AAAS=="yes", ], family = binomial)

model.sel(model.aaas.s, model.aaas.t, model.aaas.st, model.aaas.i)
```

```{r}
summary(model.aaas.i)
```


### HHMI and AAAS
We can try to add some data, and include the AAAS dataset as well. 

```{r}
model.hhmiaaas.s <- glm(formula = wh ~ sex, data=t.science[t.science$HHMI=="yes" | t.science$AAAS=="yes", ], family = binomial)
model.hhmiaaas.t <- glm(formula = wh ~ TED, data=t.science[t.science$HHMI=="yes" | t.science$AAAS=="yes", ], family = binomial)
model.hhmiaaas.st <- glm(formula = wh ~ sex + TED, data=t.science[t.science$HHMI=="yes" | t.science$AAAS=="yes", ], family = binomial)
model.hhmiaaas.i <- glm(formula = wh ~ sex * TED, data=t.science[t.science$HHMI=="yes" | t.science$AAAS=="yes", ], family = binomial)

model.sel(model.hhmiaaas.s, model.hhmiaaas.t, model.hhmiaaas.st, model.hhmiaaas.i)
```

```{r}
summary(model.hhmiaaas.i)
```
...
```{r}
summary(glm(formula = wh ~ sex+TED, data=t.science[t.science$HHMI=="yes" | t.science$AAAS=="yes", ], family = binomial))
```
...
### Actors
I downloaded on 2016-06-06 a [list of the 100 most valuable movie stars](http://www.vulture.com/2012/07/most-valuable-movie-stars.html), assigned sexes based on first names and pictures. The list contains `r sum(t.actors$sex=="F")` women names and `r sum(t.actors$sex=="M")` men names. [(download the list of names)](actors.csv).

### Scientists
I downloaded on 2016-06-06 three datasets: 

- The names of all AAAS members in the [Biological Sciences Class (II)](https://www.amacad.org/multimedia/pdfs/classlist.pdf) (`r sum(t.aaas$sex=="F")` women, `r sum(t.aaas$sex=="M")` men, [download the list of names](aaas.csv).);

- The names of all [HHMI scientists](https://www.hhmi.org/scientists/browse?kw=&sort_by=field_scientist_last_name&sort_order=ASC&items_per_page=100) (`r sum(t.hhmi$sex=="F")` women, `r sum(t.hhmi$sex=="M")` men, [download the list of names](hhmi.csv).);

- The names of all TED speakers in the [Science section](https://www.ted.com/talks?sort=popular&topics%5B%5D=science&language=en) (`r sum(t.ted$sex=="F")` women, `r sum(t.ted$sex=="M")` men, [download the list of names](ted.csv).).

Sexes were assigned on the basis of first names and pictures (apoligies if I made mistakes). 
...
```
for a total of `r length(t.science$sex)` individual scientists (`r sum(t.science$sex=="F")` women and `r sum(t.science$sex=="M")` men). 

...
getdata <- function(catname){
	...
	t2 <- cbind(t1, wh=1*((t1$wife==1)|(t1$husband==1)), sex=(t1[,2]=="F")) # F=1, M=0
...
```{r}
summary(glm(formula = wh ~ as.factor(sex), data=t.actors, family = binomial))
```
...
```{r}
summary(glm(formula = wh ~ as.factor(sex), data=t.tennis, family = binomial))
```
...
```{r}
summary(glm(formula = wh ~ as.factor(sex), data=t.science, family = binomial))
```
...
```{r}
model.s <- glm(formula = wh ~ as.factor(sex), data=t.science[t.science$HHMI==1,], family = binomial)
summary(model.s)
```
...
```{r}
model.st <- glm(formula = wh ~ as.factor(sex) + TED, data=t.science[t.science$HHMI==1, ], family = binomial)
summary(model.st)
```

```{r}
model.i <- glm(formula = wh ~ as.factor(sex) * TED, data=t.science[t.science$HHMI==1, ], family = binomial)
summary(model.i)
```

...
To answer this question, I gathered names of high profile people with different professions (scientists, tennis players, Hollywood actors), wrote a script to fetch the list of Google suggestions, and considered the proportions of people for whom "husband" and/or "wife", and "married" are suggested. 
...
```
for a total of `r length(t.science$sex)` individual scientists (`r sum(t.science$sex=="F")` women and `r sum(t.science$sex=="M")` men). The `wh` column above combines the `wife` and `husband` information (`1` means that the words "wife" and/or "husband" appear in the suggestions). 
...
dissectscience <- dissectscience / matrix(rep(c(sum(sciencesubset$sex=="F"), sum(sciencesubset$sex=="M")), 2), byrow=TRUE, ncol=2)
dissectscience <- dissectscience[, c("M", "F")] # Invert the two columns
...
par(las=1)
barplot(dmod, beside=FALSE, col=matrix(c(colMT, colM, colFT, colF), ncol=2), border=TRUE, legend=c("Men, TED", "other men", "Women, TED", "Other women"), ylim=c(0, 0.1), names.arg=c("Men", "Women"), main="Proportion of AAAS and HHMI scientists for whom \n'wife' and/or 'husband' appear as a suggestion in a Google search", cex.main=0.85)
```
...
par(mar=c(4, 7, 4, 6))
par(las=1)
barplot(dmod, beside=FALSE, col=matrix(c(colMT, colM, colFT, colF), ncol=2), border=TRUE, ylim=c(0, 0.1), names.arg=c("Men", "Women"), main="Proportion of all AAAS members in Class II: Biological Sciences\n and HHMI scientists for whom 'wife' and/or 'husband' \nappear as a suggestion in a Google search", cex.main=1)
thecex <- 1.15
...
In terms of proportions for each sex, this corresponds to
```{r echo=FALSE}
dissectscience2 <- dissectscience2 / matrix(rep(c(sum(sciencesubset$sex=="F"), sum(sciencesubset$sex=="M")), 2), byrow=TRUE, ncol=2)
dissectscience2 <- dissectscience2[, c("M", "F")] # Invert the two columns
...
par(las=1)
barplot(dmod2, beside=FALSE, col=matrix(c(colMT, colM, colFT, colF), ncol=2), border=TRUE, legend=c("Men, TED", "other men", "Women, TED", "Other women"), ylim=c(0, 0.1), names.arg=c("Men", "Women"), main="Proportion of AAAS and HHMI scientists for whom \n'married' appears as a suggestion in a Google search", cex.main=0.85)
```
